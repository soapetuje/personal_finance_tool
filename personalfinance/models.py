...
class Profile(models.Model):
    ...
    age_group = models.IntegerField(blank=True, default=UNDER_10, choices=AGE_CHOICES)
    ethnicity = models.CharField(max_length=2, choices=ETHNICITY_CHOICES, default='OT')
    race = models.CharField(max_length=2, choices=RACE_CHOICES, default='OT')
    education = models.CharField(max_length=2, choices=EDUCATION_CHOICES, default='OT')
    currency = models.CharField(max_length=3, choices=CURRENCY_CHOICES, default='EUR', blank=True)
    income = models.DecimalField(default=0, max_digits=200, decimal_places=8)
    assets = models.DecimalField(default=0, max_digits=200, decimal_places=8)
    debts = models.DecimalField(default=0, max_digits=200, decimal_places=8)

    def __str__(self):
        return f"{self.author}'s Profile"

class Price(models.Model):
    created = models.CharField(max_length=1, blank=False)
    updated = models.DateField(auto_now=True)
    response = models.CharField(max_length=5000)  

    def __str__(self):
        return f"Forex Price"

# class Income(models.Model):
#     user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='income')
#     euro_amount = models.IntegerField(default=0)
